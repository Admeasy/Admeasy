...
`relative flex items-center py-3 rounded-xl transition-all duration-300 group overflow-hidden ${isCollapsed ?"justify-center px-0":"px-4 gap-3.5"} ${isActive ?"text-white shadow-md shadow-[#9f3562]/30":"text-gray-700 hover:bg-gray-50 hover:text-[#9f3562]"}`
 ...
 {item.label}
  ...
  <div className="mt-auto pt-6 border-t border-gray-100 px-3 mb-4">
    {loggedInAccount ? (
        ...
        <div className={`flex flex-col rounded-xl hover:bg-gray-50 transition-colors duration-300 group py-2.5 ${isCollapsed ? 'items-center px-0' : 'px-3'}`}>
          ...
          <div onClick={() => navigate('/me')} className={`flex items-center w-full cursor-pointer z-10 ${isCollapsed ? 'justify-center' : 'gap-3 mb-3'}`}>
            <div className="relative flex-shrink-0">
              <ProfileCompletionCircle percentage={completion} size={isCollapsed ? 48 : 46} strokeWidth={2.5}>
                <div className="relative">
                  <img src={loggedInAccount.imageUrl || loggedInAccount.image || 'https://api.dicebear.com/7.x/avataaars/svg?seed=User'} alt={loggedInAccount.name} className={`${isCollapsed ? 'w-10 h-10' : 'w-9.5 h-9.5'} rounded-full object-cover transition-all`} />
                  <div className="absolute bottom-0 right-0 bg-white rounded-full p-[1px] shadow-sm transform translate-x-1 translate-y-1">
                    <div className="bg-[#9f3562] text-white text-[7.5px] font-bold px-1 py-0 rounded-full min-w-[17px] text-center border-[1px] border-white">
                      {completion}%
                    </div>
                  </div>
                </div>
