...
const EXAM_CTA_STORAGE_KEY = 'admeasy:examCTA:state';
const DISMISSED_KEY = 'admeasy:examCTA:dismissed';
const SCROLL_THRESHOLD = 300; // Show after scrolling 300px
const TIME_THRESHOLD = 30 * 1000; // 30 seconds of scrolling without interaction

const AddExamInfoCTA = () => {
  ...
  // Only show for users, not mentors
  const isLoggedIn = !!user && !mentor;

  // Check if user has exams
  const hasExams = user?.examsPreparingFor && Array.isArray(user.examsPreparingFor) && user.examsPreparingFor.length > 0;

  // Check if CTA should be shown
  const shouldShowCTA = () => {
    // Don't show if user is not logged in, is a mentor, or already has exams
    if (!isLoggedIn || hasExams) return false;
    ...
    const lastShown = sessionStorage.getItem(EXAM_CTA_STORAGE_KEY);
  ...
  }, [isDismissed, isLoggedIn, hasExams]);
  ...
  // Don't show if user has exams or is dismissed
  if (!isLoggedIn || hasExams || isDismissed) return null;
