...
const AddCollegeForm = ({ onClose, onSubmit, editData = null }) => {
    ...
    const generateKeywords = (data) => {
        ...
        // Add first letters of college name as keywords (both uppercase and lowercase)
        if (data.name && data.name.trim()) {
            // Split by comma first to exclude city names, then by spaces
            const nameParts = data.name.trim().split(',')[0]; // Take only the part before comma
            ...
            if (words.length > 0) {
                ...
                if (firstLetters.length > 0) {
                    keywords.add(firstLetters.toUpperCase()); // Add uppercase version
                    keywords.add(firstLetters.toLowerCase()); // Add lowercase version
                }
            }
        }
    ...
    const addStudent = () => {
        setFormData(prev => ({
            ...prev,
            students: [...prev.students, { name: '', course: '', image: null, keywords: [] }]
        }));
    };
    ...
    const addStudentKeyword = (studentIndex, keyword) => {
        if (keyword.trim()) {
            setFormData(prev => {
                const newStudents = [...prev.students];
                if (!newStudents[studentIndex].keywords) {
                    newStudents[studentIndex].keywords = [];
                }
                if (!newStudents[studentIndex].keywords.includes(keyword.trim())) {
                    newStudents[studentIndex].keywords.push(keyword.trim());
                }
                return {
                    ...prev,
                    students: newStudents
    ...
    const removeStudentKeyword = (studentIndex, keywordToRemove) => {
        setFormData(prev => {
            const newStudents = [...prev.students];
            newStudents[studentIndex].keywords = newStudents[studentIndex].keywords.filter(
