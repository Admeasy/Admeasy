...
export function processMentions(htmlContent) {
  ...
  return parts.map((part, index) => {
    ...
    if (part.startsWith('<')) {
      ...
      if (part.toLowerCase().startsWith('<iframe') && part.toLowerCase().includes('sandbox=')) {
        // It has a sandbox attribute, let's ensure it has the required permissions
        return part.replace(/sandbox=(['"])(.*?)\1/i, (match, quote, content) => {
          ...
          return `sandbox=${quote}${Array.from(permissions).join(' ')}${quote}`;
        });
      }
      return part;
    }
