...
export default function MentorProfile() {
  ...
  const isOwnProfile = !!(
    currentMentor &&
    mentor &&
    (currentMentor._id?.toString() === mentor._id?.toString() ||
      currentMentor.username === mentor.username)
  );
  useEffect(() => {
    if (contextLoading) {
      return;
    ...
    const fetchMentorProfile = async () => {
      ...
      try {
        ...
        const isOwnProfileCheck =
          !username || (currentMentor && currentMentor.username === username);
          ...
          if (mentorData.image) {
            try {
              const imageRes = await fetch(
                `/api/mentors/${mentorData._id}/pic`,
              );
              if (imageRes.ok) {
    ...
    const fetchPosts = async () => {
      ...
      try {
        const response = await fetch(
          `/api/posts/mentor/${mentor._id}?page=1&limit=10`,
          {
            credentials: "include",
          },
        );
  ...
  useEffect(() => {
    if ((user || currentMentor) && mentor && mentor._id && !isOwnProfile) {
      fetch(`/api/users/${mentor._id}/follow-status`, {
        credentials: "include",
      })
        .then((res) => res.json())
        .then((data) => {
  ...
  const handleFollow = async () => {
    ...
    if (isFollowingLoading || !mentor || !mentor._id) return;

    const previousFollowing = isFollowing;
    setIsFollowing(!isFollowing);
    setIsFollowingLoading(true);

    fetch(`/api/users/${mentor._id}/follow`, {
      method: "POST",
      credentials: "include",
    })
      .then((res) => res.json())
      ...
      .catch(() => setIsFollowing(previousFollowing))
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto px-4">
          ...
          <p className="text-gray-600 mb-6">
            {error || "The mentor profile you are looking for does not exist."}
          </p>
          <Link
            to="/mentors"
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden relative">
          ...
          <div className="px-4 sm:px-6 pb-6">
            ...
            <div className="space-y-3">
              ...
              {/* Bio */}
              {mentor.bio && (
                <p className="text-sm sm:text-base text-gray-700 leading-relaxed whitespace-pre-wrap mb-2">
                  {mentor.bio}
                </p>
              )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <motion.button
                      ...
                      className={`flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm transition-all disabled:opacity-50 shadow-md ${
                        isFollowing
                          ? "bg-gray-100 text-gray-700 hover:bg-gray-200 border border-gray-200"
                          : "bg-gradient-to-r from-[#9f3562] to-[#b14270] text-white hover:shadow-lg border border-transparent"
                      }`}
                    >
                      {isFollowingLoading ? (
                    ...
                    <motion.button
                      ...
                      onClick={() => {
                        ...
                        if (currentMentor) {
                          navigate(`/mentor/chats/${mentor._id}`);
                        } else {
                          navigate(`/chats/${mentor._id}`);
                        }
                      }}
                      className="flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-[#9f3562] to-[#b14270] text-white rounded-xl font-semibold text-sm transition-all hover:shadow-lg shadow-md"
                    >
                  ...
                  <motion.button
                    ...
                    onClick={() => {
                      ...
                      if (mentor._id) {
                        navigate(`/subscription-plans?mentorId=${mentor._id}`);
                      }
                    }}
                    className="w-full py-3 bg-gradient-to-r from-[#9f3562] to-[#b14270] text-white rounded-xl font-semibold text-sm transition-all hover:shadow-lg shadow-md flex items-center justify-center gap-2"
                  >
                    <CreditCard size={18} />
                    Subscribe
                  </motion.button>
