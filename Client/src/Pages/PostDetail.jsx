...
import { Heart, MessageCircle, Share2, ExternalLink, Youtube, ArrowLeft, Send, Trash2, Reply, UserPlus, UserCheck, Repeat2 } from 'lucide-react';
...
const PostDetail = () => {
  ...
  useEffect(() => {
    const handlePostInteraction = (event) => {
      const { postId: eventPostId, isLiked, likesCount, isReposted, repostCount, commentsCount } = event.detail;
  ...
  const handleRepost = async (e) => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('postInteraction', {
        detail: { postId, isReposted: wasReposted, repostCount: postState.repostCount }
      }));
    ...
    fetch(`/api/posts/${postId}/comment`, {
      ...
      .then(data => {
        if (data.success) {
          setComments(prev =>
            prev.map(comment =>
              comment._id === tempCommentId ? { ...data.comment, replies: [] } : comment
            )
          );
          setPostState(prev => ({ ...prev, commentsCount: data.commentsCount }));
          window.dispatchEvent(new CustomEvent('postInteraction', {
            detail: { postId, commentsCount: data.commentsCount }
          }));
        } else {
          setComments(prev => prev.filter(c => c._id !== tempCommentId));
          setPostState(prev => ({ ...prev, commentsCount: Math.max(0, (prev?.commentsCount || 0) - 1) }));
          window.dispatchEvent(new CustomEvent('postInteraction', {
            detail: { postId, commentsCount: Math.max(0, (postState.commentsCount || 0) - 1) }
          }));
  ...
  const handleReplySubmit = async (commentId) => {
    ...
    setPostState(prev => ({ ...prev, commentsCount: (prev?.commentsCount || 0) + 1 }));
    setReplyingTo(null);
    setReplyContent('');
      ...
      .then(data => {
        if (data.success) {
          setComments(prev =>
            prev.map(comment => {
              if (comment._id === commentId) {
                return {
                  ...comment,
                  replies: comment.replies.map(reply => reply._id === tempReplyId ? data.reply : reply),
                };
              }
              return comment;
            })
          ...
          setComments(prev =>
            prev.map(comment => {
              if (comment._id === commentId) {
                return { ...comment, replies: comment.replies.filter(r => r._id !== tempReplyId) };
              }
              return comment;
            })
          );
    ...
    fetch(`/api/posts/${postId}/comments/${commentId}/like`, {
      ...
      .then(data => {
        if (data.success) {
          setComments((prev) =>
            prev.map((comment) => {
              if (comment._id === commentId) return { ...comment, isLiked: data.isLiked, likesCount: data.likesCount };
              if (comment.replies && comment.replies.some(reply => reply._id === commentId)) {
                return {
                  ...comment,
                  replies: comment.replies.map((reply) =>
                    reply._id === commentId ? { ...reply, isLiked: data.isLiked, likesCount: data.likesCount } : reply
                  ),
                };
              }
              return comment;
          ...
          setComments((prev) =>
            prev.map((comment) => {
              if (comment._id === commentId) return { ...comment, isLiked: previousLiked, likesCount: previousCount };
              if (comment._id === parentCommentId) {
                return {
                  ...comment,
                  replies: comment.replies.map((reply) =>
                    reply._id === commentId ? { ...reply, isLiked: previousLiked, likesCount: previousCount } : reply
                  ),
                };
              }
              return comment;
      ...
      .then(data => {
        if (data.success) {
          ...
          if (isReply && parentCommentId) {
            setComments(prev =>
              prev.map(comment =>
                comment._id === parentCommentId ? { ...comment, replies: [...(comment.replies || []), deletedComment] } : comment
              )
            );
          } else {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-pink-50/30 flex items-center justify-center relative overflow-hidden">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-64 sm:w-96 h-64 sm:h-96 bg-purple-400/5 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
        ...
        <div className="flex flex-col items-center gap-3 sm:gap-4 relative z-10 px-4">
          ...
          <p className="text-gray-600 font-medium text-sm sm:text-base">Loading post...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-pink-50/40 relative overflow-x-hidden">
        ...
        <div className="w-full max-w-3xl px-2 sm:px-4 md:px-6 py-0 sm:py-2 md:py-12">
          ...
          >
            <div className="flex items-center gap-2 sm:gap-3 md:gap-4 p-3 sm:p-4 md:p-6 border-b border-gray-100 flex-wrap">
              <Link to={`/${postState.mentor?.username || postState.author?.username}`} className="relative flex-shrink-0">
                <img
                  src={(postState.mentor || postState.author)?.image || fallbackProfilePic}
                  alt={(postState.mentor || postState.author)?.name || 'User'}
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-gray-900 text-sm sm:text-base md:text-lg cursor-pointer hover:text-[#9f3562] transition-colors"
                  onClick={(e) => {
                    e.stopPropagation();
                    const username = (postState.mentor || postState.author)?.username;
                    if (username) navigate(`/${username}`);
                  }}>
                  {(postState.mentor || postState.author)?.name || 'User'}
                </h3>
                {(postState.mentor || postState.author)?.username && (
                  <p className="text-xs sm:text-sm text-gray-500 cursor-pointer hover:text-[#9f3562] transition-colors" onClick={(e) => {
                    e.stopPropagation();
                    navigate(`/${(postState.mentor || postState.author).username}`);
                  }}>@{(postState.mentor || postState.author).username}</p>
                )}
                {postState.mentor?.tagline && (
                  <p className="text-xs sm:text-sm text-gray-600 mt-0.5 sm:mt-1 line-clamp-1">{postState.mentor.tagline}</p>
                )}
              </div>
              {viewer && (postState.mentor?._id || postState.author?._id) &&
                viewer._id &&
                (postState.mentor?._id || postState.author?._id).toString() !== viewer._id.toString() && (
                  <motion.button
                  ...
                  >
                    {isFollowingLoading ? (
                      ...
                      <>
                        ...
                        <span className="hidden min-[360px]:inline">Following</span>
                      </>
                    ) : (
                      <>
                ...
                <div className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4 md:p-5 bg-gradient-to-r from-gray-50 to-white group-hover:from-[#9f3562]/5 group-hover:to-pink-50/50 transition-all">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-xs sm:text-sm md:text-base text-gray-900 line-clamp-1">
                      {postState.externalLink.preview?.title || postState.externalLink.preview?.domain || 'External Link'}
                    </p>
            ...
            <div className="flex items-center gap-4 sm:gap-6 md:gap-8 px-3 sm:px-4 md:px-6 py-3 sm:py-4 md:py-5 border-t border-gray-100">
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                onClick={handleLike}
                className={`flex items-center gap-1.5 sm:gap-2.5 transition-colors ${postState.isLiked ? 'text-red-500' : 'text-gray-600 hover:text-red-500'}`}
              >
                <Heart className={`w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 ${postState.isLiked ? 'fill-current' : ''}`} />
                <span className="text-sm sm:text-base md:text-lg font-bold">{postState.likesCount}</span>
              </motion.button>
              <div className="flex items-center gap-1.5 sm:gap-2.5 text-gray-600">
                ...
                <span className="text-sm sm:text-base md:text-lg font-bold">{postState.commentsCount}</span>
              </div>
              <motion.button
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
                onClick={handleRepost}
                className={`flex items-center gap-1.5 sm:gap-2.5 transition-colors ${postState.isReposted ? 'text-[#9f3562]' : 'text-gray-600 hover:text-[#9f3562]'}`}
              >
                <Repeat2 className={`w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 ${postState.isReposted ? 'fill-current' : ''}`} />
                {postState.repostCount > 0 && (
                  <span className="text-sm sm:text-base md:text-lg font-bold">{postState.repostCount}</span>
                )}
              </motion.button>
              <motion.button
            ...
            <div className="border-t border-gray-100 px-3 sm:px-4 md:px-6 py-3 sm:py-4 md:py-6 bg-gray-50/50">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="bg-white rounded-xl sm:rounded-2xl px-3 sm:px-4 py-2 sm:py-3 shadow-sm border border-gray-100">
                          {viewer && comment.user?._id === viewer._id ? (
                            <Link to={`/me`} onClick={(e) => e.stopPropagation()} className="block">
                        ...
                        <div className="flex items-center gap-2 sm:gap-3 md:gap-4 mt-1.5 sm:mt-2 ml-2 sm:ml-4 flex-wrap">
                          <button
                            ...
                            className={`flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs font-medium transition-colors disabled:opacity-50 ${comment.isLiked ? 'text-red-500' : 'text-gray-500 hover:text-red-500'}`}
                          >
                          ...
                          <span className="text-[10px] sm:text-xs text-gray-500">{formatDate(comment.createdAt)}</span>
                          {viewer && comment.user?._id === viewer._id && (
                            <button
                        ...
                        {comment.replies && comment.replies.length > 0 && (
                          <div className="mt-2 sm:mt-3 ml-2 sm:ml-4 space-y-2 sm:space-y-3 border-l-2 border-gray-100 pl-2 sm:pl-4">
                            {comment.replies.map((reply) => (
                              <div key={reply._id} className="flex items-start gap-2 sm:gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-2 sm:gap-3 mt-1 sm:mt-1.5 ml-2 sm:ml-3 flex-wrap">
                                    <button
                                      ...
                                      className={`flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs font-medium transition-colors disabled:opacity-50 ${reply.isLiked ? 'text-red-500' : 'text-gray-500 hover:text-red-500'}`}
                                    >
                                      <Heart className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${reply.isLiked ? 'fill-current' : ''}`} />
                        ...
                        {replyingTo === comment._id && (
                          <div className="mt-2 sm:mt-3 ml-2 sm:ml-4 flex gap-1.5 sm:gap-2 flex-wrap">
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleReplySubmit(comment._id); }
                            ...
                            <button onClick={() => { setReplyingTo(null); setReplyContent(''); }} className="px-2 sm:px-3 py-1.5 sm:py-2 text-gray-600 hover:text-gray-800 transition-colors text-xs sm:text-sm">Cancel</button>
                          </div>
                        )}
                      </div>
                    </motion.div>
              ...
              <form onSubmit={handleCommentSubmit} className="flex items-start gap-2 sm:gap-3">
                <img
                ...
                <div className="flex-1 flex gap-1.5 sm:gap-2">
                  ...
                  >
                    {isSubmittingComment ? <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 animate-spin" /> : <Send className="w-4 h-4 sm:w-5 sm:h-5" />}
                  </motion.button>
                </div>
              </form>
            </div>
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          ...
          handleCommentDelete(confirmModal.commentId, confirmModal.isReply, confirmModal.parentCommentId);
        }}
        title="Delete Comment"
        message={confirmModal.isReply ? "Are you sure you want to delete this reply? This action cannot be undone." : "Are you sure you want to delete this comment? This action cannot be undone."}
        confirmText="Delete"
        cancelText="Cancel"
        confirmColor="danger"
