  ...
  const handleGoogleClick = async () => {
    ...
    try {
      if (shouldUseCapacitorGooglePlugin()) {
        ...
        window.location.href = getWebGoogleOAuthUrl();
      }
    } finally {
    ...
    >
      ...
      <div className="mt-6">
        <button
          ...
          className={`flex items-center justify-center gap-3 w-full bg-white border-2 border-gray-300 text-gray-700 rounded-full text-base px-4 py-3 hover:bg-gray-50 transition-all shadow-md hover:shadow-lg ${googleLoading ? 'opacity-70 cursor-not-allowed' : ''}`}
        >
