...
router.post("/signup", limits.strict, async (req, res) => {
  try {
    ...
    await user.save();
    // ── REFERRAL LINKAGE ──────────────────────────────────
    const { referralCode: usedCode } = req.body;
    if (usedCode) {
      try {
        const referrer = await User.findOne({
          referralCode: usedCode.toUpperCase().trim(),
        });
        ...
        console.error("Referral linkage error:", referralErr);
      ...
      if (user.googleId) {
        ...
        if (!req.cookies["accessToken"]) {
          return res.status(401).json({
            success: false,
            message: "Please log in with Google to complete onboarding.",
          });
        }
  ...
  async (req, res) => {
    try {
      ...
      if (onboardingStatus.requiresOnboarding) {
        res.redirect(
          `${frontendUrl}/onboarding?oauth_success=true&token=${accessToken}&${switchParam}`,
        );
      } else {
        res.redirect(
          `${frontendUrl}/?oauth_success=true&token=${accessToken}&${switchParam}`,
        );
      }
    } catch (err) {
