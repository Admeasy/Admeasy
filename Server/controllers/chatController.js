...
const PostSchemaFile = require("../models/postSchema");
// const Post = require("../models/postSchema"); // Import Post model using Admeasy connection
...
const Post =
  Admeasy.models.Post ||
  Admeasy.model("Post", PostSchemaFile.schema || PostSchemaFile);
    ...
    const formattedChats = await Promise.all(
      chats.map(async (chat) => {
        try {
          ...
          const mentor = await Mentor.findById(chat.mentorId)
...
const getUserToMentorChatMessages = async (req, res) => {
  try {
    ...
    if (userId) {
      ...
      await UserToMentorChat.updateOne({ _id: chatId }, { userUnreadCount: 0 });
    } else if (mentorId) {
      ...
      messages.map(async (message) => {
        try {
          ...
          if (message.senderRole === "user") {
            ...
            sender = await Mentor.findById(message.senderId)
              .select("name image username")
              .lean();
          }

          // --- B. Fetch Post Data (The Cross-DB Bridge) ---
          let postData = null;
          if (message.messageType === "post" && message.postId) {
            try {
              // Post model points to the Admeasy DB
              postData = await Post.findById(message.postId).lean();
            } catch (postErr) {
          ...
          if (!sender) {
            return {
              ...
              messageType: message.messageType || "text", // Added this
              postId: message.postId || null, // Added this
              post: postData, // Added this
          ...
          return {
            ...
            senderRole:
              message.senderRole ||
              (message.senderRole === "mentor" ? "mentor" : "user"),
          ...
          console.error(
            `Error fetching sender ${message.senderId}:`,
            senderError,
          );
          return {
            _id: message._id,
...
const sendUserToMentorMessage = async (req, res) => {
  try {
    const { message, messageType = "text", postId } = req.body;
    const chatId = req.userToMentorChat._id;

    //Indentify who is sending the message (user or mentor) and get their ID
    const senderId =
      req.user?._id || req.user?.id || req.mentor?._id || req.mentor?.id;
    ...
    if (messageType === "post") {
      if (!postId) {
        return res.status(400).json({
          success: false,
          message: "postId is required for post type messages",
        });
      }
    }
    ...
    // --- 3. Update Chat Inbox Metadata ---
    const lastMessagePreview =
      messageType === "text" ? message.trim() : "📌 Shared a post";
    ...
    if (senderRole === "user") {
      ...
      sender = await Mentor.findById(senderId)
        .select("name image username")
        .lean();
    }
    ...
    (async () => {
      try {
        const recipientId =
          senderRole === "user"
            ? req.userToMentorChat.mentorId
            : req.userToMentorChat.userId;
...
const createOrGetUserToMentorChat = async (req, res) => {
  try {
    console.log(
      "createOrGetUserToMentorChat - req.user:",
      req.user ? "exists" : "missing",
      "req.mentor:",
      req.mentor ? "exists" : "missing",
    );
  ...
  try {
    ...
    if (!chat) {
      return res.status(404).json({
        success: false,
        message:
          "Chat not found. You can only access chats that have been initiated by users.",
      });
    }
...
const getUserToUserChats = async (req, res) => {
  try {
    ...
    // REMOVED: const UserModel = Users.model("Users");
    ...
    const formattedChats = await Promise.all(
      chats.map(async (chat) => {
        try {
          ...
          const otherUserId =
            chat.user1Id.toString() === userId.toString()
              ? chat.user2Id
              : chat.user1Id;

          // CHANGED: Using the 'User' model directly instead of UserModel
          ...
          console.error(
            `Error fetching user ${chat.user1Id === userId ? chat.user2Id : chat.user1Id}:`,
            error,
          );
          return null;
        }
...
//       await UserToUserChat.updateOne({ _id: chatId }, { user1UnreadCount: 0 });
//     } else {
//       await UserToUserChat.updateOne({ _id: chatId }, { user2UnreadCount: 0 });
//     }

//     // Fetch sender data
//     // const UserModel = Users.model("Users"); // No longer needed since we're importing User model directly at the top
//     const formattedMessages = await Promise.all(
//       messages.map(async (message) => {
//         try {
//           //   const sender = await UserModel.findById(message.senderId) //replace it with below as we are importing User model directly
...
//               //added postId and messageType to the response for frontend rendering
//               //changed sendername to "Deleted User" and senderImage to null as message were not being fetched
//               _id: message._id,
//               senderId: message.senderId,
//               senderRole: "user",
...
//               senderImage: null,
//               message: message.message,
//               messageType: message.messageType || "text", //added
//               post: message.postId || null, //added
//               status: message.status,
//               createdAt: message.createdAt,
//               readAt: message.readAt,
//             };
...
const getUserToUserChatMessages = async (req, res) => {
  try {
    ...
    // 1️⃣ Fetch messages
    const messages = await UserToUserMessage.find({ chatId })
      .sort({ createdAt: 1 })
      .lean();
    ...
    // 3️⃣ Reset unread count for current user
    const chat = await UserToUserChat.findById(chatId);
    if (chat.user1Id.toString() === userId.toString()) {
      await UserToUserChat.updateOne({ _id: chatId }, { user1UnreadCount: 0 });
    } else {
    ...
    // // 4️⃣ Fetch sender data and post data manually
    // const formattedMessages = await Promise.all(
    //   messages.map(async (message) => {
    //     // Fetch post only if message has postId
    //     let post = null;
    //     if (message.postId) {
    //       try {
    //         post = await Post.findById(message.postId.toString()).lean(); // use Admeasy connection
    //       } catch (postError) {
    //         console.error(`Error fetching post ${message.postId}:`, postError);
    //       }
    //     }
    ...
    //       senderId: message.senderId,
    //       senderRole: "user",
    //       senderName: sender?.name || sender?.username || "Deleted User",
    //       senderImage: sender?.image || null,
    //       message: message.message,
    //       messageType: message.messageType || "text",
    //       post: post || null, // populated manually
    //       status: message.status,
    //       createdAt: message.createdAt,
    //       readAt: message.readAt,
    //     };
    ...
    const formattedMessages = await Promise.all(
      messages.map(async (message) => {
        // Look for the ID in both common field names
        const targetPostId = message.postId || message.post;

        let postData = null;
        if (targetPostId) {
          try {
            // Using the Admeasy model we defined at the top of the file
            postData = await Post.findById(targetPostId.toString()).lean();
          } catch (postError) {
            console.error(`Error fetching post ${targetPostId}:`, postError);
          }
        }
        ...
        return {
          ...
          message: message.message,
          messageType: message.messageType || "text",
          post: postData, // Will be the full object from Admeasy DB or null
          status: message.status,
          createdAt: message.createdAt,
          readAt: message.readAt,
        };
...
const sendUserToUserMessage = async (req, res) => {
  try {
    ...
    const newMessage = new UserToUserMessage({
      ...
      // FIX: Allow message to be saved for both "text" and "post" types
      message: message ? message.trim() : null,
      messageType,
      postId: messageType === "post" ? postId : null,
      status: "sent",
    ...
    const chat = await UserToUserChat.findById(chatId);
    const unreadField =
      chat.user1Id.toString() === senderId.toString()
        ? "user2UnreadCount"
        : "user1UnreadCount";
    await UserToUserChat.updateOne(
      { _id: chatId },
...
const createOrGetUserToUserChat = async (req, res) => {
  try {
    console.log(
      "createOrGetUserToUserChat - req.user:",
      req.user ? "exists" : "missing",
    );
    const otherUserId = req.params.userId;
    const userId = req.user._id || req.user.id;
    console.log(
      "createOrGetUserToUserChat - userId:",
      userId,
      "otherUserId:",
      otherUserId,
    );
    ...
    // const UserModel = User.model("Users"); // No longer needed since we're importing User model directly at the top
    const otherUser = await User.findById(otherUserId);
    if (!otherUser) {
      return res.status(404).json({
        success: false,
    ...
    const userIds = [userId, otherUserId].sort((a, b) =>
    ...
    const otherUserInfo =
      user1Id.toString() === userId.toString()
        ? await User.findById(user2Id).select("name username image").lean()
        : await User.findById(user1Id).select("name username image").lean();
    ...
    if (error.code === 11000) {
      try {
        const userIds = [req.user._id || req.user.id, req.params.userId].sort(
        ...
        if (existingChat) {
          //   const UserModel = Users.model("Users"); // No longer needed since we're importing User model directly at the top
          const otherUserId =
            userIds[0].toString() === (req.user._id || req.user.id).toString()
              ? userIds[1]
              : userIds[0];
          const otherUser = await User.findById(otherUserId)
            .select("name username image")
    ...
    const formattedChats = await Promise.all(
      chats.map(async (chat) => {
        try {
          ...
          const otherMentorId =
            chat.mentor1Id.toString() === mentorId.toString()
              ? chat.mentor2Id
              : chat.mentor1Id;
          ...
          console.error(
            `Error fetching mentor ${chat.mentor1Id === mentorId ? chat.mentor2Id : chat.mentor1Id}:`,
            error,
          );
          return null;
        }
...
const getMentorToMentorChatMessages = async (req, res) => {
  try {
    ...
    const formattedMessages = await Promise.all(
      messages.map(async (message) => {
        try {
          const sender = await Mentor.findById(message.senderId)
            .select("name image username")
            .lean();

          //fetch post data if message has postId
          let postData = null;
          if (message.messageType == "post" && message.postId) {
            try {
              postData = await Post.findById(message.postId.toString())
                .select("content image externalLink.preview")
                .lean(); //fetching only necessary fields for preview
            } catch (postError) {
              console.error(
                `Error fetching post ${message.postId}:`,
                postError,
              );
            }
          }
          ...
          return {
            ...
            messageType: message.messageType || "text",
            postId: message.postId || null,
            postData: postData, // will be null for non-post messages
            status: message.status,
            createdAt: message.createdAt,
            readAt: message.readAt,
          };
...
const sendMentorToMentorMessage = async (req, res) => {
  try {
    ...
    const chat = await MentorToMentorChat.findById(chatId);
    const unreadField =
      chat.mentor1Id.toString() === senderId.toString()
        ? "mentor2UnreadCount"
        : "mentor1UnreadCount";
    await MentorToMentorChat.updateOne(
      { _id: chatId },
...
const createOrGetMentorToMentorChat = async (req, res) => {
  try {
    ...
    const mentorIds = [mentorId, otherMentorId].sort((a, b) =>
      a.toString().localeCompare(b.toString()),
    );
    ...
    const otherMentorInfo =
      mentor1Id.toString() === mentorId.toString()
    ...
    if (error.code === 11000) {
      try {
        ...
        if (existingChat) {
          const otherMentorId =
            mentorIds[0].toString() ===
            (req.mentor._id || req.mentor.id).toString()
              ? mentorIds[1]
              : mentorIds[0];
          const otherMentor = await Mentor.findById(otherMentorId)
            .select("name username image")
...
const getMentorChats = async (req, res) => {
  try {
    ...
    // Combine and sort by lastMessageTime
    const allChats = [...mentorToUserChats, ...mentorToMentorChats].sort(
      (a, b) => {
        const timeA = a.lastMessageTime
          ? new Date(a.lastMessageTime).getTime()
          : 0;
