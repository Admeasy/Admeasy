...
exports.likeNote = async (req, res) => {
  ...
  if (req.user?._id) {
    const note = await Note.findById(req.params.id)
      .select("tags hashtags title description")
      .lean();
    trackStudentEvent({
      userId: req.user._id,
      eventType: "note_like",
...
exports.shareNote = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Unable to track share." });
  }
};
...
exports.uploadNote = async (req, res) => {
  ...
  try {
    ...
    const isSchoolNote =
      req.body.schoolNotes === "true" || req.body.schoolNotes === true;
    ...
    // Log note_uploaded for mentors only (fire-and-forget)
    if (uploaderType === "Mentor") {
      const { logMentorActivity } = require("../utils/logMentorActivity");
      logMentorActivity(uploader._id, "note_uploaded", {
        noteId: note._id,
        title: note.title,
      });
      ...
      MentorModel.findByIdAndUpdate(uploader._id, {
        lastActiveAt: new Date(),
      }).catch((err) =>
        console.error("[uploadNote] lastActiveAt update failed:", err),
      );
    }
    res.status(201).json({
      success: true,
